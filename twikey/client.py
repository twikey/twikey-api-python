...
class TwikeyClient(object):
    ...
    def __init__(self, api_key, base_url="https://api.twikey.com", user_agent="twikey-python/v0.1.0") -> None:
    ...
    def refreshTokenIfRequired(self):
        ...
        if self.lastLogin == None or (now - self.lastLogin).seconds > 23 * 3600:
            ...
            response = requests.post(self.instance_url(), data=payload, headers={"User-Agent": self.user_agent})
            if "ApiErrorCode" in response.headers:
                # print response.headers
                raise Exception(
                    "Error authenticating : %s - %s" % (response.headers["ApiErrorCode"], response.headers["ApiError"]))
    ...
    def logout(self):
        ...
        if "ApiErrorCode" in response.headers:
            # print response.headers
            raise Exception(
                "Error logging out : %s - %s" % (response.headers["ApiErrorCode"], response.headers["ApiError"]))
